...
int chevauche_carres(Une_coord x1, Une_coord y1,Une_coord x2, Une_coord y2){
    ...
    if(point_est_dans_carre(x1, x2, y2) || point_est_dans_carre(y1, x2, y2) || point_est_dans_carre(z1, x2, y2) || point_est_dans_carre(d1, x2, y2)){
        return 1;
    }
    return 0;
}
...
Un_elem *chercher_zone(Un_noeud *aqr, Un_elem *liste, Une_coord limite_se, Une_coord limite_no){
    ...
    Une_coord c1 = {.lat = limite_se.lat, .lon = limite_mil.lon};
    Une_coord c2 = {.lat = limite_mil.lat, .lon = limite_no.lon};

    // Coordonnées du carré au Sud Ouest
    Une_coord c3 = {.lat = limite_mil.lat, .lon = limite_se.lon};
    Une_coord c4 = {.lat = limite_no.lat, .lon = limite_mil.lon};

    if(chevauche_carres(limite_no, limite_se, aqr->limite_no, limite_mil)){
        liste = chercher_zone(aqr->no, liste, limite_se, limite_no);
    }

    if(chevauche_carres(limite_no, limite_se, limite_mil, aqr->limite_se)){
        liste = chercher_zone(aqr->se, liste, limite_se, limite_no);
    }

    if(chevauche_carres(limite_no, limite_se, c2, c1)){
        liste = chercher_zone(aqr->ne, liste, limite_se, limite_no);
    }

    if(chevauche_carres(limite_no, limite_se, c4, c3)){
