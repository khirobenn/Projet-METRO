...
int point_est_dans_carre(Une_coord point, Une_coord x, Une_coord y){
    if(point.lon <= y.lon && point.lon >= x.lon && point.lat <= x.lat && point.lat >= y.lat){
        return 1;
    }
    return 0;
}
...
int chevauche_carres(Une_coord x1, Une_coord y1,Une_coord x2, Une_coord y2){
    Une_coord z1 = {.lat = y1.lat, .lon = x1.lon};
    Une_coord d1 = {.lat = x1.lat, .lon = y1.lon};
    Une_coord z2 = {.lat = y2.lat, .lon = x2.lon};
    Une_coord d2 = {.lat = x2.lat, .lon = y2.lon};
...
Un_elem *chercher_zone(Un_noeud *aqr, Un_elem *liste, Une_coord limite_se, Une_coord limite_no){
    ...
    Une_coord nord_est_no = {.lat = aqr->limite_no.lat, .lon = limite_mil.lon};
    Une_coord nord_est_se = {.lat = limite_mil.lat, .lon = aqr->limite_se.lon};

    // Coordonnées du carré au Sud Ouest
    Une_coord sud_ouest_no = {.lat = limite_mil.lat, .lon = aqr->limite_no.lon};
    Une_coord sud_ouest_se = {.lat = aqr->limite_se.lat, .lon = aqr->limite_no.lon};
